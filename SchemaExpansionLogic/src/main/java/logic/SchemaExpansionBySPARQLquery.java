...
public class SchemaExpansionBySPARQLquery implements SchemaExpansion{
	...
	public SchemaExpansionBySPARQLquery(Set<Predicate> knownPredicates, Set<Rule> rules) {
		this.knownPredicates = knownPredicates;
		this.rules = rules;
		this.additionalVocabularies = null;
	}
	
	public SchemaExpansionBySPARQLquery(Set<Predicate> knownPredicates, Set<Rule> rules, Model additionalVocabularies) {
		this.knownPredicates = knownPredicates;
		this.rules = rules;
		this.additionalVocabularies = additionalVocabularies;
	}
	
	public SchemaExpansionBySPARQLquery() {
	...
	public Set<PredicateInstantiation> expand(int approach, Set<PredicateInstantiation> existingPredicates, boolean consistencyCheck, StatRecorder sr) {
		if(approach == 0) return expandSCORE(existingPredicates, consistencyCheck, sr);
		else if (approach == 1) return expandCritical(existingPredicates, consistencyCheck, sr);
		else if (approach == 2) return expandSCORE2(existingPredicates, consistencyCheck, sr);
		else if (approach == 3) return expandSCOREwithFilters(existingPredicates, consistencyCheck, sr);
	...
	public Set<PredicateInstantiation> expandCritical(Set<PredicateInstantiation> existingPredicates, boolean consistencyCheck, StatRecorder sr) {
		...
		newPredicates.addAll(expandSCORE(newKnownPredicates,consistencyCheck,sr));
	...
	public Set<PredicateInstantiation> expandSCORE(Set<PredicateInstantiation> existingPredicates, boolean consistencyCheck, StatRecorder sr) {
	...
	public Set<PredicateInstantiation> expandSCOREwithFilters(Set<PredicateInstantiation> existingPredicates, boolean consistencyCheck, StatRecorder sr) {
		statinconsistencycheck = 0;
		statinconsistencycheckfound = 0;
		statinconsistencycheckreused = 0;
		overallConsistencyChecks = 0;
		...
		newKnownPredicates.addAll(existingPredicates);
		newKnownPredicates.addAll(newPredicates);
		newPredicates.addAll(expandSCOREwithFilters(newKnownPredicates,consistencyCheck,sr));
	...
	public Set<PredicateInstantiation> expandSCORE2(Set<PredicateInstantiation> existingPredicates, boolean consistencyCheck, StatRecorder sr) {
		statinconsistencycheck = 0;
		statinconsistencycheckfound = 0;
		statinconsistencycheckreused = 0;
		overallConsistencyChecks = 0;
		...
		newKnownPredicates.addAll(existingPredicates);
		newKnownPredicates.addAll(newPredicates);
		newPredicates.addAll(expandSCORE2(newKnownPredicates,consistencyCheck,sr));
